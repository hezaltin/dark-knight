...
export const createReducer = config => {
  ...
  return (state = {}, action) => {
    switch (action.type) {
      ...
      case types.UPDATE_RESULT_BOOKMARK: {
        const uri = action.payload.uri
        const bookmarked = action.payload.bookmarked
        const results = Object.assign([], state.response.results)
        let targetResult = results.find(x => x.uri === uri)
        if (!targetResult) {
          return state
        }
        targetResult.matches.bookmarked = bookmarked
        return {
          ...state,
          response: {
            ...state.response,
