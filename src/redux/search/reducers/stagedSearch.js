...
export const createReducer = config => {
  ...
  const advancedQuery = (state = {}, action) => {
    switch (action.type) {
      case types.SET_ADVANCED_QUERY:
        return action.payload.advancedQuery
      default:
        return state
    }
  }
          ...
          return state.map(filter => {
            if (filter === existingFilter) {
              return {
                ...filter,
                value: [
                  ...(action.type === types.REPLACE_FILTER ? [] : filter.value),
...
export const selectors = {
  ...
  getAdvancedQuery: state => state.advancedQuery,
