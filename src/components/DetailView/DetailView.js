  ...
  return (
    <div className="tab-level-inner">
      <Tabs
        ...
        id="manufacturers"
        animation={false}
        activeKey={childTabValue}
        bsStyle="pills"
        onSelect={props.handleChangeLevel}
        id="noanim-tab-example"
      >
        {data
          ? data['ui:headingOrder'].map((el, i) => {
              return (
                <Tab
                  key={i}
                  eventKey={i}
                  title={
                    dataProperties.properties[el]
                      ? dataProperties.properties[el].title
                      : el
                  }
                  target={`${el}`}
                >
                  <TabLevelContent
                    ...
                    parentTabValue={props.parentTabValue}
                    childTabvalue={childTabValue}
                    dataPropertiesFormats={dataPropertiesFormats}
                    dataPropertiesSet={dataPropertiesSet}
                  ></TabLevelContent>
                </Tab>
              );
            })
...
const TabLevelContent = props => {
  ...
  const getSelectedKeyValue = Object.keys(
    dataPropertiesFormats[parentTabValue].properties
  )[childTabvalue];
    ...
    <div className="detail-list-container">
      <div className="detail-request">
        {properties ? (
          ...
          <ItemLevelContent
            attributes={TabLevelData[getSelectedKeyValue]}
            data={allData}
          ></ItemLevelContent>
        )}
      </div>
      ...
      if (attributes['format'] === 'data-url') {
        switch (attributes['compas:preview']) {
          case 'image': {
            return (
              <div>
                <img src={'/v1/documents?uri=' + data.uri} height="120"></img>
              </div>
            );
          }
          case 'docs': {
            ...
            return (
              <div>
                <a
                  href={'localhost:9003/v1/documents?uri=' + data.uri}
                  target="__blank"
                >
                  {data.name}
                </a>
  ...
  return (
    <div className="detail-list-container">
      ...
      <div className="detail-request">
        {data ? (
          data.map((el, i) => (
            <div key={uniqueKey()} className="doc-container">
              <div>
                <i
                  key={uniqueKey()}
                ...
                <a
                  ...
                  onClick={$event => clickHandler($event, el.uri)}
                  key={uniqueKey()}
                >
                  {' '}
                  {el.uri.split('/').pop()}{' '}
  ...
  return (
    <div className="detail-list-container">
      ...
      <div className="detail-request">
        ...
        <div>
          <span>VONTRON ULP32-8040 Membrane Element</span>
        </div>
      </div>
    </div>
  );
...
class DetailView extends React.Component {
  ...
  setDivSize() {
    this.setState({
      width: this.selector.current.getBoundingClientRect().width
    });
  }
  ...
  createMetadataPanel() {
    ...
    const dataPropertiesSet = viewReferenceData['ui:tabOrder'].reduce(
      (all, item, index) => {
        return {
    ...
    return (
      <Col xs={12} md={12} className="ml-result-metadata">
        ...
        <div>
          ...
          >
            {instance.summary.productInfo.model} (
            {instance.summary.productInfo.manufacturer})
          </h1>
        </div>
        ...
        <div className="detail-page">
          <div className="detail-tabs">
            <Tabs
              defaultActiveKey={viewReferenceData['ui:tabOrder'][0]}
              animation={false}
              activeKey={value}
              ...
              {viewReferenceData['ui:tabOrder'].map((el, i) => {
                return (
                  <Tab
                    key={i}
                    eventKey={el}
                    title={dataPropertiesFormat[el].title}
                  >
                    <TabsLevelInnerMaterial
                      childTabValue={tabLevel}
                      data={viewReferenceData[el]}
                      uri={this.uri}
                      handleChangeLevel={this.handleChangeTabLevel}
                      dataProperties={dataPropertiesFormat[el]}
                      dataPropertiesSet={dataPropertiesSet}
                      dataPropertiesFormats={dataPropertiesFormat}
  ...
  createPdfPreviewPanel() {
    ...
    import('react-file-viewer')
      .then(({ FileViewer }) => {
        return (
          <Col xs={12} md={7} key={uniqueKey()} className="ml-doc-preview">
            <div
              style={{
                width: '100%',
                display: 'flex',
            ...
            >
              ...
              <div
                ref={this.selector}
                style={{ width: '100%', height: '700px', margin: '50px 0' }}
                key={uniqueKey()}
              >
  ...
  render() {
    ...
    return (
      <div key={uniqueKey()}>
        <Row key={uniqueKey()}>{this.createMetadataPanel()}</Row>
