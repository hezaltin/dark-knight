...
} from '../../../libs/JsonSchemaForm/TypeaheadSelector/MultiObjectSelector'
import { 
  AsyncStringSelectorField 
} from '../../../libs/JsonSchemaForm/TypeaheadSelector/StringSelector'
import {
  ArrayItemRichTextEditorField
} from '../../../libs/JsonSchemaForm/RichTextEditor/ArrayItemRichTextEditor'
import {
  FileUploaderField
} from '../../../libs/JsonSchemaForm/FileUploader/FileUploader'
...
const CustomSchemaField = function (props) {
  ...
  if (name && schema) {
    ...
    customProps.onChange = (formData) => {
      ...
      switch (!schema['brain:value'] && schema.type === 'array' && schema.items['brain:value'] && schema.items['brain:value']['brain:valueType']) {
        case 'attribute': 
          formContext.onAttributeChange(name, formData, schema['brain:xpath'])
          break
...
export default class EditEntry extends Component {
    ...
    componentDidUpdate(prevProps) { 
      ...
      if ((isEmpty(prevProps.doc) && !isEmpty(this.props.doc)) || !isEqual(prevProps.doc, this.props.doc)) {
        this.setState({doc: this.props.doc})
      }

      if ((!prevProps.group && !!this.props.group) || prevProps.group !== this.props.group) {
      ...
      if ((this.props.selection && !isEmpty(this.props.selection))
        ...
        let dropdownList = Object.keys(schema['properties'])
          ...
          .map((key) => {
            ...
            return {
              key,
              title: el['title'],
              children: el['properties'] && Object.keys(el['properties'])
                .map((keyChild) => {
        ...
        let sideBarProps = Object.keys(schema['properties'])
          .map((el) => ({[el]: schema['properties'][el]}))
        let sideBarList = {}
        sideBarProps.forEach(el => {
          sideBarList[Object.keys(el)[0]] = Object.keys(el[Object.keys(el)[0]]['properties'])
        })
    
        this.setState({dropdownList, sideBarList, selectedSchema: selectedSchema, propertySelected: "summary"})
      }
    }
    ...
    render() {
      ...
      // console.log({dataSchema, uiSchema})
        ...
        >
          <div style={{ textAlign: "left"}}>
            <button className="btn btn-primary" type="submit" 
              disabled={this.props.checkPending || this.props.duplicate || !this.state.unsaved} 
              style={{ marginRight: "6px" }}>Save</button>
            {/* <span>{
              this.state.unsaved && 'Unsaved changes'   
            }</span> */}
          </div>
        </Form>
      </>
